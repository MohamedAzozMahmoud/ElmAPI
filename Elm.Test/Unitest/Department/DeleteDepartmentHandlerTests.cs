using Elm.Application.Contracts;
using Elm.Application.Contracts.Features.Department.Commands;
using Elm.Application.Contracts.Repositories;
using Elm.Application.Features.Department.Handlers;
using Moq;

namespace Elm.Test.Unitest.Department
{
    public class DeleteDepartmentHandlerTests
    {
        private readonly Mock<IDepartmentRepository> _repositoryMock;
        private readonly DeleteDepartmentHandler _handler;

        public DeleteDepartmentHandlerTests()
        {
            _repositoryMock = new Mock<IDepartmentRepository>();
            _handler = new DeleteDepartmentHandler(_repositoryMock.Object);
        }

        [Fact]
        public async Task Handle_WhenDeleteSucceeds_ReturnsSuccessWithTrue()
        {
            // Arrange
            var departmentId = 1;
            var command = new DeleteDepartmentCommand(departmentId);

            _repositoryMock.Setup(r => r.DeleteAsync(departmentId))
                .ReturnsAsync(true);

            // Act
            var result = await _handler.Handle(command, CancellationToken.None);

            // Assert
            Assert.True(result.IsSuccess);
            Assert.True(result.Data);
            _repositoryMock.Verify(r => r.DeleteAsync(departmentId), Times.Once);
        }

        [Fact]
        public async Task Handle_WhenDeleteFails_ReturnsFailure()
        {
            // Arrange
            var departmentId = 999;
            var command = new DeleteDepartmentCommand(departmentId);

            _repositoryMock.Setup(r => r.DeleteAsync(departmentId))
                .ReturnsAsync(false);

            // Act
            var result = await _handler.Handle(command, CancellationToken.None);

            // Assert
            Assert.False(result.IsSuccess);
            Assert.Equal("Failed to delete department", result.Message);
            Assert.Equal(500, result.StatusCode);
            _repositoryMock.Verify(r => r.DeleteAsync(departmentId), Times.Once);
        }

        [Theory]
        [InlineData(1)]
        [InlineData(50)]
        [InlineData(100)]
        public async Task Handle_WithVariousIds_CallsRepositoryWithCorrectId(int departmentId)
        {
            // Arrange
            var command = new DeleteDepartmentCommand(departmentId);

            _repositoryMock.Setup(r => r.DeleteAsync(departmentId))
                .ReturnsAsync(true);

            // Act
            var result = await _handler.Handle(command, CancellationToken.None);

            // Assert
            Assert.True(result.IsSuccess);
            _repositoryMock.Verify(r => r.DeleteAsync(departmentId), Times.Once);
        }

        [Fact]
        public async Task Handle_WhenDepartmentNotFound_ReturnsFailure()
        {
            // Arrange
            var nonExistentId = 99999;
            var command = new DeleteDepartmentCommand(nonExistentId);

            _repositoryMock.Setup(r => r.DeleteAsync(nonExistentId))
                .ReturnsAsync(false);

            // Act
            var result = await _handler.Handle(command, CancellationToken.None);

            // Assert
            Assert.False(result.IsSuccess);
            Assert.Equal("Failed to delete department", result.Message);
        }

        [Fact]
        public async Task Handle_RepositoryCalledExactlyOnce()
        {
            // Arrange
            var departmentId = 1;
            var command = new DeleteDepartmentCommand(departmentId);

            _repositoryMock.Setup(r => r.DeleteAsync(departmentId))
                .ReturnsAsync(true);

            // Act
            await _handler.Handle(command, CancellationToken.None);

            // Assert
            _repositoryMock.Verify(r => r.DeleteAsync(It.IsAny<int>()), Times.Once);
            _repositoryMock.Verify(r => r.DeleteAsync(departmentId), Times.Once);
        }
    }
}
